...
public class UserLinkDao extends BasicDaoImpl<UserLink> {
	...
	public UserLink getlatestdate(String broadcasterUserId) {
		Query query = new Query();
		// query.limit(10);
		query.addCriteria(Criteria.where("broadcasterUserId").is(broadcasterUserId));
		...
		/*
		 * 
		 * System.out.println(" KJSJHFJKSH>>>>>>>>>>" +
		 * getMongoTemplate().find(query, UserLink.class)); UserLink userlink =
		 * getMongoTemplate().findOne(query, UserLink.class);
		 * System.out.println("Get Date Broadcasted" +
		 * userlink.getLinkbroadcastingDate()); return userlink;
		 */
	}
